...
CREATE TABLE `kkpay_order`  (
  ...
  `profit_amount` decimal(11, 2) NULL DEFAULT NULL COMMENT '订单利润',
  ...
  `trade_state` enum('WAIT_PAY','TRADE_CLOSED','TRADE_SUCCESS','TRADE_REFUND','TRADE_FINISHED','TRADE_FROZEN') CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT 'WAIT_PAY' COMMENT '交易状态',
  ...
  INDEX `idx_mch_trade_no`(`mch_trade_no` ASC) USING BTREE,
...
CREATE TABLE `kkpay_order_refund`  (
  ...
  `refund_fee_amount` decimal(11, 2) NULL DEFAULT 0.00 COMMENT '退还服务费金额',
  `fee_bearer` bit(1) NOT NULL DEFAULT b'0' COMMENT '服务费承担方 0:平台 1:商户',
...
CREATE TABLE `kkpay_payment_channel`  (
  `id` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `code` varchar(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '通道编码',
  ...
  `gateway` varchar(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '网关代码(插件类名)',
  `cost` decimal(5, 4) NOT NULL DEFAULT 0.0000 COMMENT '费率成本',
  `fixed_cost` decimal(12, 2) NOT NULL DEFAULT 0.00 COMMENT '固定成本',
  `rate` decimal(5, 4) NOT NULL DEFAULT 0.0000 COMMENT '费率',
  `fixed_fee` decimal(12, 2) NOT NULL DEFAULT 0.00 COMMENT '固定服务费',
  `min_fee` decimal(12, 2) NULL DEFAULT NULL COMMENT '最低服务费',
  `max_fee` decimal(12, 2) NULL DEFAULT NULL COMMENT '最高服务费',
  `min_amount` decimal(12, 2) UNSIGNED NULL DEFAULT NULL COMMENT '单笔最小金额',
  `max_amount` decimal(12, 2) UNSIGNED NULL DEFAULT NULL COMMENT '单笔最大金额',
  `daily_limit` decimal(12, 2) UNSIGNED NULL DEFAULT NULL COMMENT '单日收款限额',
...
CREATE TABLE `kkpay_payment_channel_account`  (
  ...
  `diy_order_subject` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '自定义商品名称(留空继承)',
...
-- ----------------------------
-- Table structure for kkpay_payment_gateway_log
-- ----------------------------
DROP TABLE IF EXISTS `kkpay_payment_gateway_log`;
CREATE TABLE `kkpay_payment_gateway_log`  (
  `id` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `trade_no` char(24) CHARACTER SET ascii COLLATE ascii_bin NULL DEFAULT NULL COMMENT '关联平台订单号',
  `gateway` varchar(16) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '网关代码',
  `method` varchar(32) CHARACTER SET ascii COLLATE ascii_bin NOT NULL COMMENT '调用方法',
